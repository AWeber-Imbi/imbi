...
- [ ] **C4.** Plugin reload is unauthenticated arbitrary code execution — `src/imbi_api/plugins/reload.py:31-43` + `src/imbi_api/plugins/installer.py:24-57`. Sign Valkey reload payloads with HMAC, allowlist `^imbi-plugin-[a-z0-9_-]+$`, pin `--index-url`, pass `--no-deps`.
...
- [ ] **H16.** OpenAPI `_schema_cache` regenerates concurrently under cold load — `src/imbi_api/openapi.py:202-280`. Wrap with `asyncio.Lock` or precompute at startup.
...
- [ ] **H21.** Admin user setup ignores membership-write outcome — `src/imbi_api/entrypoint.py:451-465`. Assert row returned or exit non-zero.
- [ ] **H22.** OpenAPI `/docs` URL ignores `api_prefix` — `src/imbi_api/app.py:81-85`, `src/imbi_api/openapi.py:424`.
...
- [ ] **M12.** `extra='allow'` on response models leaks internals — `src/imbi_api/domain/models.py:679, 922, 1126`. Switch to `extra='ignore'`.
...
- [ ] **M14.** PII in login-failure logs — `endpoints/auth.py:277, 288, 299, 309` log full emails. Hash/redact.
- [ ] **M15.** `authenticate_api_key` round-trips per request even when throttled — `auth/permissions.py:455, 477`. Add a per-process LRU.
- [ ] **M16.** `find_or_create_oauth_identity` does 5 graph round-trips — `endpoints/auth.py:1156-1207`.
- [ ] **M17.** `dispatch_lifecycle` ignores plugin `enabled` flag — `plugins/lifecycle_dispatch.py:79-101`, `plugins/resolution.py:228`.
- [ ] **M18.** `revoke_connection` swallows IdP-side failures — `identity/flows.py:417-434`. Surface failure in the API response.
...
- [ ] **M30.** OIDC discovery cache process-local and unbounded — `auth/oauth.py:17`. `functools.lru_cache(maxsize=64)` or Valkey.
- [ ] **M31.** OAuth state JWT is 10-min TTL and not single-use — `auth/oauth.py:97`. Add a delete-on-use nonce.
- [ ] **M32.** Rate-limit gaps: `/mfa/verify` (TOTP brute), `/auth/logout`, `/auth/oauth/{p}/callback` (outbound amplification), `/auth/token` client-id scanning.
...
- [ ] **M36.** `check_resource_permission` resource-type → label mapping is brittle — `auth/permissions.py:606-624`, mapping at 678 (`project_logs` → `ProjectLogs` vs actual `ProjectLog`).
...
- [ ] **M39.** `_fetch_current_releases` fully parses every event to find latest timestamp — `endpoints/projects.py:550-555`. Use the lighter `_latest_deployment_timestamp`.
...
- [ ] **M44.** `_load_plugin_handler` falls back from id to slug match — `identity/flows.py:46-83`. Make disambiguation explicit.
...
- [ ] **L9.** Org slug rename doesn't store `previous_slugs` for redirect resolution — `endpoints/organizations.py:316-403`.
- [x] **L10.** `patch.py:9` indirect `__import__('logging')`. Replace with normal import. — landed on `main` (6c9a365).
- [ ] **L11.** `relationships.py:11-23` accepts `dict[str, tuple[str, int]]` — switch to a `NamedTuple` to prevent positional swaps.
- [ ] **L12.** `entrypoint.py:53-80, 86-118, 289-326` reimplements Cypher templating that `graph_sql.py` provides.
- [x] **L13.** `entrypoint.py:218, 222` `raise typer.Exit(code=1) from None` is cargo-culted; remove. — landed on `main` (6c9a365).
- [ ] **L14.** OpenAPI generator has 5 `except Exception:` swallows — `openapi.py:90, 146, 244, 256, 271`. Bust cache on partial failure.
...
- [ ] **L17.** `_replace_state` doesn't validate HTTPS — `identity/flows.py:182-191`.
- [ ] **L18.** `score_history_feed` filters out non-existent empty-string `change_reason` — `endpoints/scoring.py:462, 499`.
...
- [ ] **L22.** Plugin-controlled URLs (`run_url`, `release_url`) stored in audit JSON; confirm UI escapes — `endpoints/project_deployments.py:417-426`.
- [ ] **L23.** `list_service_applications` `usage='login'` drops `org_slug` scoping — `endpoints/third_party_services.py:705-734`. Document or add dedicated permission.
- [ ] **L24.** `_handle_deploy` writes audit row even when no release matched — `endpoints/project_deployments.py:1086-1097`.
...
- [ ] **L27.** `parse_scopes` accepts any string — `auth/permissions.py:438`. Validate against seeded `Permission` set.
