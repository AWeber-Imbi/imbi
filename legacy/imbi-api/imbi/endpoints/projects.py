...
class CollectionRequestHandler(_ProjectRequestMixin,
    ...
    INSERT INTO v1.projects 
                (namespace_id, project_type_id, created_by,  "name", slug, 
                 description, data_center, environments, deployment_type,
                 configuration_system, orchestration_system)
         VALUES (%(namespace_id)s, %(project_type_id)s, %(username)s, %(name)s, 
                 %(slug)s, %(description)s, %(data_center)s, %(environments)s,
                 %(deployment_type)s, %(configuration_system)s,
                 %(orchestration_system)s)
      RETURNING id""")
