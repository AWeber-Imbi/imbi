...
| `mcp.py`             | Shared policy for building AI toolsets from the Imbi OpenAPI spec — `EXCLUDED_ROUTE_MAPS` (static denylist) and `exclude_non_ai_tools` (`route_map_fn` honoring the `x-imbi-ai-tool: false` extension). Requires the `mcp` extra (`fastmcp`). Consumed by `imbi-mcp`/`imbi-assistant` |
...
| `otel.py`            | `TraceIdResponseMiddleware` — ASGI middleware that writes the active OpenTelemetry trace ID to a configurable response header (default `trace-id`). Also exposes `current_trace_id()` returning the active trace ID as a hex string (used by `AccessLogMiddleware`). No-op when `opentelemetry-api` is not installed or no valid span is active. |
...
| `access_log.py`      | `AccessLogMiddleware` ASGI middleware emitting NCSA-style access logs on `imbi_common.access`; honors `quiet_paths`, extracts the principal from `Authorization`, prepends `trace_id:<hex>` to the suffix when an OpenTelemetry span is active, and appends `(k:v ...)` context from `request.state.imbi_common_access_log` |
