...
from imbi_api import entrypoint, models, settings, version
...
class ServeTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.server_config = settings.ServerConfig()
        self.mock_config = self.enterContext(
            mock.patch.object(
                entrypoint.settings, 'ServerConfig', lambda: self.server_config
            )
        )
        self.mock_uvicorn_run = self.enterContext(
            mock.patch.object(entrypoint.uvicorn, 'run')
        )

    def test_serve_production_mode(self) -> None:
        """Test serve in production mode."""
        # Configure environment
        self.server_config.environment = 'production'

        # Call the function
        self.runner.invoke(entrypoint.main, ['serve'])

        # Verify uvicorn.run was called
        self.mock_uvicorn_run.assert_called_once()
        call_args = self.mock_uvicorn_run.call_args
        ...
        self.assertEqual(kwargs['host'], self.server_config.host)
        self.assertEqual(kwargs['port'], self.server_config.port)
    ...
    def test_serve_development_mode(self) -> None:
        """Test serve in development mode."""
        # Configure environment
        self.server_config.environment = 'development'
    ...
    def test_serve_with_dev_flag(self) -> None:
        """Test serve with dev=True flag."""
        # Configure production environment
        self.server_config.environment = 'production'

        # Call the function with --dev flag
        self.runner.invoke(entrypoint.main, ['serve', '--dev'])

        # Verify uvicorn.run was called with reload even in production
        call_args = self.mock_uvicorn_run.call_args
        kwargs = call_args[1]

        # --dev should enable reload regardless of environment
        self.assertTrue(kwargs['reload'])
        self.assertIn('reload_dirs', kwargs)
    ...
    def test_serve_custom_host_port(self) -> None:
        ...
        self.server_config.environment = 'production'
        self.server_config.host = '0.0.0.0'
        self.server_config.port = 9000
