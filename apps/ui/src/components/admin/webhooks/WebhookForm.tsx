...
import { Save, X, AlertCircle, Plus, Trash2, ArrowUp, ArrowDown } from 'lucide-react'
...
export function WebhookForm({
  ...
  const [notificationPath, setNotificationPath] = useState(webhook?.notification_path || '/')
  const [secret, setSecret] = useState('')
  const [tpsSlug, setTpsSlug] = useState(webhook?.third_party_service?.slug || '')
  const [identifierSelector, setIdentifierSelector] = useState(webhook?.identifier_selector || '')
  ...
  const validate = () => {
    ...
    if (slug && !/^[a-z][a-z0-9-]*$/.test(slug)) {
      newErrors.slug = 'Slug must start with a letter and contain only lowercase letters, numbers, and hyphens'
    }
    if (!notificationPath.trim()) newErrors.notification_path = 'Notification path is required'
    ...
    if (identifierSelector && !tpsSlug) {
      newErrors.identifier_selector = 'Identifier selector requires a third-party service'
    }
  ...
  const addRule = () => {
    setRules([...rules, { filter_expression: '', handler: '', handler_config: {} }])
  }
  ...
  const updateRuleField = (index: number, field: 'filter_expression' | 'handler', value: string) => {
  ...
  const updateRuleConfig = (index: number, jsonStr: string) => {
    try {
      ...
      setRules(rules.map((r, i) => (i === index ? { ...r, handler_config: parsed } : r)))
      setErrors((prev) => { const next = { ...prev }; delete next[`rule_${index}_config`]; return next })
    } catch {
      setErrors((prev) => ({ ...prev, [`rule_${index}_config`]: 'Invalid JSON' }))
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {isEditing ? 'Update webhook configuration' : 'Configure a new inbound webhook'}
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className={`p-6 rounded-lg border ${
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className={`block text-sm mb-1.5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  Secret {isEditing && <span className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>(leave blank to keep current)</span>}
                </label>
                <Input
                  type="password"
                  value={secret}
                  onChange={(e) => setSecret(e.target.value)}
                  placeholder={isEditing ? '(unchanged)' : 'HMAC verification secret'}
                  disabled={isLoading}
                  className={isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''}
                />
              </div>
        ...
        <div className={`p-6 rounded-lg border ${
          ...
          <p className={`mb-4 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Optionally link this webhook to a third-party service for automatic project resolution.
          </p>
            ...
            {tpsSlug && (
              <div>
                ...
                <Input
                  ...
                  disabled={isLoading}
                  className={`font-mono text-sm ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''} ${
                ...
                <p className={`mt-1 text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                  JSON Path expression to extract the project identifier from the webhook payload.
                </p>
              </div>
            )}
          </div>
        ...
        <div className={`p-6 rounded-lg border ${
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Define filter expressions and handlers. Rules are evaluated in order.
              </p>
            </div>
            <Button
              type="button"
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          <label className={`block text-xs mb-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                            Filter Expression (CEL) <span className="text-red-500">*</span>
                          </label>
                          <Input
                            value={rule.filter_expression}
                            onChange={(e) => updateRuleField(index, 'filter_expression', e.target.value)}
                            placeholder='e.g., body.action == "push"'
                            disabled={isLoading}
                            className={`font-mono text-sm ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''} ${
                              errors[`rule_${index}_filter`] ? 'border-red-500' : ''
                            }`}
                          />
                        ...
                        <div>
                          ...
                          <Input
                            value={rule.handler}
                            onChange={(e) => updateRuleField(index, 'handler', e.target.value)}
                            placeholder="e.g., imbi_gateway.handlers.sync_project"
                            disabled={isLoading}
                            className={`font-mono text-sm ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : ''} ${
                              errors[`rule_${index}_handler`] ? 'border-red-500' : ''
                            }`}
                          />
                      ...
                      <div>
                        ...
                        <textarea
                          defaultValue={JSON.stringify(rule.handler_config, null, 2)}
                          onBlur={(e) => updateRuleConfig(index, e.target.value)}
                          rows={2}
                          disabled={isLoading}
                          placeholder="{}"
