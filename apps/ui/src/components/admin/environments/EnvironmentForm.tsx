...
export function EnvironmentForm({
  ...
  const [dynamicFormData, setDynamicFormData] = useState<Record<string, unknown>>(
    environment ? extractDynamicFields(environment, ENVIRONMENT_BASE_FIELDS_SET) : {}
  )
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {isEditing ? 'Update environment information' : 'Create a new environment'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isLoading ? 'Saving...' : isEditing ? 'Save Changes' : 'Create Environment'}
          </Button>
        </div>
      </div>
